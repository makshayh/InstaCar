	...
	<div class="card" style="width: 150rem;">
		...
		<div class="card-body">
			...
			<div class="container" id="right-panel">
				<form method="POST" action="${contextPath}/booking"
					modelAttribute="booking" class="form-signin">
					<h2 class="form-heading">Select Route</h2>

					<div class="form-group ${error != null ? 'has-error' : ''}">
						<span>${message}</span> <input id="source" name="source"
							type="text" class="form-control" placeholder="source"
							value="Bangalore" /> <input id="destination" name="destination"
							type="destination" class="form-control" placeholder="Destination"
							onfocusout="demo()" /> <span>${error}</span> <input type="hidden"
							name="${_csrf.parameterName}" value="${_csrf.token}" /> <input
							type="hidden" id="fare" name="fare" value=""> <span>${error}</span>
						<input type="hidden" name="${_csrf.parameterName}"
							value="${_csrf.token}" /> <input type="date" id="date"
							onkeydown="return false" required name="date" placeholder="date"
